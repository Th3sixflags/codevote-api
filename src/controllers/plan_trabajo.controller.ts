...
export async function listar(req: Request, res: Response) {
  const registros = await service.listarPlanTrabajo(req.user?.fk_id_institucion);
  res.json(registros);
}

export async function obtener(req: Request, res: Response) {
  const registro = await service.obtenerPlanTrabajo(Number(req.params.id), req.user?.fk_id_institucion);
  if (!registro) {
...
export async function crear(req: Request, res: Response) {
  ...
  const nuevo = await service.crearPlanTrabajo(data, req.user?.fk_id_institucion);
  res.status(201).json(nuevo);
}

export async function actualizar(req: Request, res: Response) {
  const data        = actualizarPlanTrabajoSchema.parse(req.body);
  const actualizado = await service.actualizarPlanTrabajo(Number(req.params.id), data, req.user?.fk_id_institucion);
  if (!actualizado) {
    res.status(404).json({ error: 'Plan de trabajo no encontrado.' });
    return;
  }
...
export async function eliminar(req: Request, res: Response) {
  const eliminado = await service.eliminarPlanTrabajo(Number(req.params.id), req.user?.fk_id_institucion);
  if (!eliminado) {
    res.status(404).json({ error: 'Plan de trabajo no encontrado.' });
    return;
  }
