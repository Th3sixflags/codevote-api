...
export async function listar(req: Request, res: Response) {
  const listas = await service.listarListas(await visibilidadListasDe(req), req.user?.fk_id_institucion);
  res.json(listas);
}
...
export async function obtener(req: Request, res: Response) {
  ...
  const lista = await service.obtenerLista(Number(req.params.id), await visibilidadListasDe(req), req.user?.fk_id_institucion);
  if (!lista) {
...
export async function listarPorProceso(req: Request, res: Response) {
  const listas = await service.listarPorProceso(Number(req.params.procesoId), await visibilidadListasDe(req), req.user?.fk_id_institucion);
  res.json(listas);
}

/** Listas que compiten en una papeleta concreta. */
export async function listarPorVotacion(req: Request, res: Response) {
  const listas = await service.listarPorVotacion(Number(req.params.votacionId), await visibilidadListasDe(req), req.user?.fk_id_institucion);
  res.json(listas);
}

export async function crear(req: Request, res: Response) {
  const data  = crearListaSchema.parse(req.body);
  const nueva = await service.crearLista(data, req.user?.fk_id_institucion);
  res.status(201).json(nueva);
}

export async function actualizar(req: Request, res: Response) {
  const data        = actualizarListaSchema.parse(req.body);
  const actualizada = await service.actualizarLista(Number(req.params.id), data, req.user?.fk_id_institucion);
  if (!actualizada) {
    res.status(404).json({ error: 'Lista no encontrada.' });
    return;
  }
...
export async function eliminar(req: Request, res: Response) {
  const eliminada = await service.eliminarLista(Number(req.params.id), req.user?.fk_id_institucion);
  if (!eliminada) {
    res.status(404).json({ error: 'Lista no encontrada.' });
    return;
  }
...
export async function aprobar(req: Request, res: Response) {
  const lista = await service.aprobarLista(Number(req.params.id), req.user?.fk_id_institucion);
  if (!lista) {
    res.status(404).json({ error: 'Lista no encontrada.' });
    return;
  }
...
export async function rechazar(req: Request, res: Response) {
  ...
  const lista = await service.rechazarLista(Number(req.params.id), motivo, req.user?.fk_id_institucion);
  if (!lista) {
    res.status(404).json({ error: 'Lista no encontrada.' });
    return;
  }
...
export async function retirar(req: Request, res: Response) {
  const lista = await service.retirarLista(Number(req.params.id), req.user?.fk_id_institucion);
  if (!lista) {
    res.status(404).json({ error: 'Lista no encontrada.' });
    return;
  }
...
export async function transferirResponsable(req: Request, res: Response) {
  ...
  const lista = await service.transferirResponsable(Number(req.params.id), cedula_nuevo_responsable, req.user?.fk_id_institucion);
  if (!lista) {
    res.status(404).json({ error: 'Lista no encontrada.' });
    return;
  }
